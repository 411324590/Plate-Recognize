...
def MagicModel(input_shape = (72, 272, 1) ):
    ...
    x = BatchNormalization(axis=3)(x)
    x = Activation('relu')(x)
    x = Conv2D(filters = 1024, kernel_size=(3, 3), padding='same',kernel_initializer=keras.initializers.glorot_uniform(seed=0))(x)
    x = BatchNormalization(axis=3)(x)
    x = Activation('relu')(x)
...
# plot_model(magicmodel, to_file='ETEModel05_Five.png',show_shapes=True)
